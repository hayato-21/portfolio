...
// パラメータ改ざんチェック
//================================
// GETパラメータはあるが、改ざんされている（URLをしくじった）
...
if(!empty($_POST)){
  ...
  // 更新の場合はDBの情報を入力情報が異なる場合にバリデーションを行う
  if(empty($dbFormData)){
      ...
      if($dbFormData['price'] !== $price){ //前回までは、キャストしていたが、ゆるい判定でも良い
          validRequired($price, 'price');
          validNumber($price, 'price');
      }
  }
  if(empty($err_msg)){
      ...
      try{
        ...
        $dbh = dbConnect();
        // SQL文を作成   
        // 編集画面の場合はUPDATE文、新規登録の場合は、INSERT文を生成
        if($edit_flg){
          debug('DB更新です。');
          $sql = 'UPDATE product SET name = :name, category_id = :category_id, price = :price, comment = :comment, pic1 = :pic1, pic2 = :pic2, pic3 = :pic3 WHERE user_id = : u_id AND id = :p_id';
          $data = array(':name' => $name, ':category_id' => $category, ':price' => $price, ':comment' => $comment, ':pic1' => $pic1, ':pic2' => $pic2, ':pic3' => $pic3, ':u_id' => $_SESSION['user_id'], ':p_id' => $p_id);
        }else{
          debug('DB新規登録です。');
