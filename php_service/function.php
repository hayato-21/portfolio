...
function debug($str){
  global $debug_flg;
  if(!empty($debug_flg)){
    error_log('デバッグ：'.$str); //開発する時だけtrueにする。
  }
}
//================================
...
function debugLogStart(){
  ...
  debug('セッション変数の中身：'.print_r($_SESSION,true)); //trueは吐き出すのオッケーという意味。
...
function validRequired($str, $key){
  if($str === ''){ //金額フォームなどを考えると数値の０はOKにし、空文字はダメにする  //emptyにすると、0とヌルの値を、確かデフォルトと取るはず。
    global $err_msg;
    $err_msg[$key] = MSG01;
  }
}
...
function validEmailDup($email){
  global $err_msg;
  //例外処理
  try { //自分でエラー時の処理を書ける。throw new Exceptionで書ける。
    // DBへ接続
    $dbh = dbConnect(); //DBの接続準備関数
    // SQL文作成
    $sql = 'SELECT count(*) FROM users WHERE email = :email AND delete_flg = 0'; //countは検索で引っかかったものはいくつあるのか。デリートフラグが立っていないものから、探す。立っている人ももう一回出来るように。
    $data = array(':email' => $email);
    // クエリ実行
    $stmt = queryPost($dbh, $sql, $data);//select文の時は、query
    // クエリ結果の値を取得
...
//バリデーション関数（最小文字数チェック）
function validMinLen($str, $key, $min = 6){ //特に入って無くても、変数の値を指定していれば、関数内で使うことが出来る。
...
//バリデーション関数（最大文字数チェック）
function validMaxLen($str, $key, $max = 256){ //もし、255文字を超えると、データベースでは、255文字までしか保存できないため、エラーになる。一般的には、DB側とアプリケーション側のダブルチェックが多い。
...
function dbConnect(){
  ...
  $password = 'root';
  $options = array(
    // SQL実行失敗時にはエラーコードのみ設定 //もし、サイレントにしなかった場合、catchの方に処理が移り、エラーメッセージが表示される。ユーザーが混乱してしまうため。だから、
...
function getUser($u_id){
  ...
  try {
    ...
    // クエリ結果のデータを１レコード返却
    if($stmt){
      return $stmt->fetch(PDO::FETCH_ASSOC);// PDO：FETCH_ASSOCはreturnの役割もある。
    }else{
      return false;
    }
...
function getProductList($currentMinNum = 1, $category, $sort, $span = 20){
  ...
  try {
    // DBへ接続
    $dbh = dbConnect();
    // 件数用のSQL文作成                  //件数だけなので、Idだけで良い。
    $sql = 'SELECT id FROM product';
    if(!empty($category)) $sql .= ' WHERE category_id = '.$category;  // SQL文の追加。
    if(!empty($sort)){
      switch($sort){
        case 1:
          $sql .= ' ORDER BY price ASC';  // SQL文の追加。
          break;
        case 2:
          $sql .= ' ORDER BY price DESC';  // SQL文の追加。
          break;
      }
    } 
    $data = array();
    ...
    $rst['total_page'] = ceil($rst['total']/$span); //総ページ数  ceilは端数を切り上げる(四捨五入関係なく)。つまり、101ページを20で割ると、5.005で、端数の切り上げで6．となる。4.3でも5になる。この場合、1件を表示させるために、6ページ目が必要だから。
    if(!$stmt){
      return false;
    }
    
    // ページング用のSQL文作成
    $sql = 'SELECT * FROM product';    // 商品の検索なので、productの情報を全て取得する。
    ...
    $sql .= ' LIMIT '.$span.' OFFSET '.$currentMinNum;  // LIMITの後ろは、何件取得するかの上限、OFFSETは、どこから値を取るのか？追加のSQL文。
    $data = array();
    debug('SQL：'.$sql);
    // クエリ実行
    $stmt = queryPost($dbh, $sql, $data);
...
function getMsgsAndBord($id){
  ...
  try {
    ...
    $dbh = dbConnect();
    // SQL文作成　m.idとは？
    $sql = 'SELECT m.id AS m_id, product_id, bord_id, send_date, to_user, from_user, sale_user, buy_user, msg, b.create_date FROM message AS m RIGHT JOIN bord AS b ON b.id = m.bord_id WHERE b.id = :id AND m.delete_flg = 0 ORDER BY send_date ASC';  // m.idはmessageテーブルの中の、idのこと。m.id(m_id)が、b.idとマッチするもの。bordの方が重要なので、RIGHTJOIN
    $data = array(':id' => $id);  //m_id=id(b.id
    // クエリ実行
    $stmt = queryPost($dbh, $sql, $data);
    if($stmt){
      // クエリ結果の全データを返却
...
function getMyMsgsAndBord($u_id){
  ...
  try {
    ...
    // クエリ実行
    $stmt = queryPost($dbh, $sql, $data);
    $rst = $stmt->fetchAll();    // 複数の人とやり取りしている可能性があるため、FETCH_ALLを使用して全て取得する。
    if(!empty($rst)){
      foreach($rst as $key => $val){
        // SQL文作成
        $sql = 'SELECT * FROM message WHERE bord_id = :id AND delete_flg = 0 ORDER BY send_date DESC';  // 最新のものを表示させたいから。全て降順にする。
...
function getFormData($str, $flg = false){
  if($flg){
    $method = $_GET;  //flgがtrueであれば、$_GETを使う。
  }else{
    $method = $_POST;
  }
  global $dbFormData;
  ...
  if(!empty($dbFormData)){
    ...
    if(!empty($err_msg[$str])){
      //POSTにデータがある場合 //0円の場合もあるので、issetは０をtrueで返す。emptyは０をないと判定する。
      if(isset($method[$str])){
      ...
      //POSTにデータがあり、DBの情報と違う場合　他の入力はあっているが、その他の入力が間違っている。名前いいけど、電話番号違う。など。
...
function pagination( $currentPageNum, $totalPageNum, $link = '', $pageColNum = 5){
  ...
  echo '<div class="pagination">';
    echo '<ul class="pagination-list">';
      ...
      for($i = $minPageNum; $i <= $maxPageNum; $i++){  // 3だったら、7まで5つ出す。
        echo '<li class="list-item ';
        if($currentPageNum == $i ){ echo 'active'; }   // このif文は処理が続く限り、trueを返す。3、4、5、6、7、というように。
        echo '"><a href="?p='.$i.$link.'">'.$i.'</a></li>';
      }
      if($currentPageNum != $maxPageNum && $maxPageNum > 1){
        echo '<li class="list-item"><a href="?p='.$maxPageNum.$link.'">&gt;</a></li>';  //最後に遷移→　２つとも２重の矢印の方が良いのでは？
      }
    echo '</ul>';
  echo '</div>';
}
