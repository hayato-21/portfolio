...
// DBから掲示板とメッセージデータを取得  だから、RIGHT_JOINか！掲示板は親で右側これは絶対に必要。メッセージデータはない場合もある。なぜ、左側にメッセージがあるのか？それは、わかりやすさ？
...
$dealUserIds[] = $viewData[0]['sale_user'];  // なぜ、keyを指定しているのか？メッセージ情報の中の同じレコードから、情報を取りたいから、keyを指定しているのではないか？ここで、分けている。
$dealUserIds[] = $viewData[0]['buy_user'];   // これなんで必要？販売履歴に使えそうな気がする。
if(($key = array_search($_SESSION['user_id'], $dealUserIds)) !== false) {  // 検索したい変数を第一引数、どこから検索するのかを第二引数に指定する。見つかった最初のキーを返す。
    unset($dealUserIds[$key]);  // falseではなかったら、変数を破棄する。?？？？？？下の処理はtrueじゃないとできない？
} 
$partnerUserId = array_shift($dealUserIds);  // array_shiftは最初の配列をとるので、sale_user。つまり相手。
    ...
    <div id="contents" class="site-width">
      ...
      <section id="main" >
        ...
        <div class="area-bord" id="js-scroll-bottom">
         <?php
            if(!empty($viewData)){
              foreach($viewData as $key => $val){   // つまり、$viewDataには、$partnerUserIdと$userが入っている？もし、そうであれば、どこかで分けているはずだ。
